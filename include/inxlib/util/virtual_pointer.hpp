...
struct virtual_pointer_next<T1> {
	using type =
	    std::conditional_t<std::is_const_v<T1>, virtual_pointer<const void>,
	                       virtual_pointer<void>>;
};
...
class virtual_pointer<T, Ts...>
    : public details::virtual_pointer_next_t<T, Ts...> {
protected:
...
protected:
	virtual_pointer(T* l_ptr, vfun_type* l_cast) noexcept
	    : super(l_ptr, l_cast) {}
	static void_type* get(void_type* ptr) noexcept {
		return static_cast<T*>(super::get(ptr));
	}
	static void_type* cast_up(void_type* ptr, const std::type_info& ti
		...
		if constexpr (!std::is_const<To>::value) {
			...
			if constexpr (std::is_same<To, const void>::value) {
				...
				return static_cast<To*>(
				    m_cast(m_ptr, typeid(std::remove_const_t<To>)));
			}
		}
	}
...
protected:
	virtual_pointer(void_type* l_ptr, vfun_type* l_cast) noexcept
	    : m_ptr(l_ptr), m_cast(l_cast) {}
	static void_type* get(void_type* ptr) noexcept { return ptr; }
	static void_type* cast_up(void_type*, const std::type_info& ti
	                          [[maybe_unused]]) noexcept {
...
public:
	...
	explicit virtual_pointer(const virtual_pointer<const void>&) noexcept =
	    default;
...
bool operator<(const virtual_pointer<VP1, VPs...>& lhs,
               const P2* rhs) noexcept {
	return std::less<std::common_type_t<const VP1*, const P2*>>(lhs.get(), rhs);
}
template <typename P2, typename VP1, typename... VPs>
bool operator>(const virtual_pointer<VP1, VPs...>& lhs,
               const P2* rhs) noexcept {
	return std::greater<std::common_type_t<const VP1*, const P2*>>(lhs.get(),
	                                                               rhs);
}
template <typename P2, typename... VPs>
bool operator<=(const virtual_pointer<VPs...>& lhs, const P2* rhs) noexcept {
